...
$("startRoom").onclick=()=>{const r=getRoom();if(!r||r.host!==selfId||Object.keys(r.players).length<2)return;r.gameId=makeGameId();r.battleStateId="";Object.values(r.players).forEach(p=>{resetPlayerForLobby(p);p.gameId=r.gameId});assignTeams(r);r.phase="select";r.deadline=now()+10000;r.endReady={};r.returnLobbyAt=0;Object.values(r.players).forEach(p=>{p.ready=false;p.element=""});if(r.players[selfId])syncLocalFromPlayer(r.players[selfId],true);saveRoom(r);showSelect()};
