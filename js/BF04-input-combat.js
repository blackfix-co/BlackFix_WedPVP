...
function cardRange(card){return arenaRadius*2+32}
...
function aimEndpoint(card=null,caster=local){const dir=aimDirection(caster),range=cardRange(card),pos=caster.pos||local.pos,origin=typeof castOrigin==="function"?castOrigin(caster):{x:pos.x,y:(pos.y||2)+.25,z:pos.z},floorY=typeof arenaFloorY==="function"?arenaFloorY():-0.9,end={x:origin.x+dir.x*range,y:floorY,z:origin.z+dir.z*range};constrainArenaPosition(end);return end}
...
function queueSkillHit(card,e,target=null){const id=local.element||"fire",dir=aimDirection(local),range=cardRange(card),origin=typeof castOrigin==="function"?castOrigin(local):{x:local.pos.x,y:local.pos.y+.25,z:local.pos.z},hit=aimEndpoint(card,local),dist=Math.hypot(hit.x-origin.x,hit.z-origin.z),delay=skillImpactDelay(id,card,dist),h={id:selfId+"-"+now()+"-"+Math.random().toString(36).slice(2,7),card:{...card},element:id,origin,hit,dir,range,targetId:"",createdAt:now(),impactAt:now()+delay*1000,delay};pendingHits.push(h);if(typeof travelTelegraphEffect==="function")travelTelegraphEffect(h,e);return h}
