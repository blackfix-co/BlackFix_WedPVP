...
function resolveSkillHit(h){const e=elements[h.element]||elements.fire,targets=targetsOnStoredPath(h),area=typeof areaTelegraphCard==="function"&&areaTelegraphCard(h.card,h.element),hitList=area?targets.filter(t=>t.impactHit):targets.slice(0,1),caster={pos:{x:h.origin.x,y:h.origin.y,z:h.origin.z},rot:{y:Math.atan2(-h.dir.x,-h.dir.z)},element:h.element,fxOrigin:true},impactCard={...h.card,resolvedImpact:true};if(hitList.length){hitList.forEach(t=>hitTarget(t,h.card,e));skillEffect(impactCard,hitList[0],e,caster,area?h.hit:hitList[0].hit)}else skillEffect(impactCard,null,e,caster,h.hit)}
