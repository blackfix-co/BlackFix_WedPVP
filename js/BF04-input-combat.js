...
function dragonTailShockwave(){const r=getRoom();if(!r)return;Object.entries(r.players).forEach(([id,p])=>{if(id===selfId||!p.alive||p.hp<=0||!p.pos)return;if(r.mode==="team"&&p.team===local.team)return;const d=Math.hypot(p.pos.x-local.pos.x,p.pos.z-local.pos.z);if(d>7)return;hitTarget({id,p,d,hit:{x:p.pos.x,y:p.pos.y+.25,z:p.pos.z}}, {name:"드래곤 꼬리 충격파",type:"CC기",damage:10,slow:1,knock:.6}, elements.dragon||elements.fire)})}
...
function startPsychicLiftControl(target,card,e){
  ...
  const timer=setInterval(()=>{
    ...
    if(slam){
      nextSlam=t+310;
      hitTarget({id:targetId,p,d:Math.hypot((p.pos.x||0)-local.pos.x,(p.pos.z||0)-local.pos.z),hit:{x:p.pos.x,y:p.pos.y+.25,z:p.pos.z}}, {name:"염동 내려찍기",type:"CC기",damage:slamDamage,stun:.1,unparryable:true,psychicSlam:true}, e);
    }
  },55);
}
