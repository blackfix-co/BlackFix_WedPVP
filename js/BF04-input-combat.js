...
function triggerAltParry(){const t=now();if(!local.alive||t<local.stunUntil)return;if(t<(local.parryCdUntil||0)){log(`패링 쿨타임 ${((local.parryCdUntil-t)/1000).toFixed(1)}초`);return}local.parryUntil=t+100;local.parryCdUntil=t+200;parryFlashEffect();patch({parryUntil:local.parryUntil});log("순간 패링")}
...
function aimEndpoint(card=null,caster=local){const pos=caster.pos||local.pos,origin=typeof castOrigin==="function"?castOrigin(caster):{x:pos.x,y:(pos.y||2)+.25,z:pos.z},floorY=typeof arenaFloorY==="function"?arenaFloorY():-0.9;if(window.BFCore?.aimEndpoint){const end=window.BFCore.aimEndpoint({pos,rot:caster.rot||local.rot,origin,floorY,arenaRadius,moveLimit});constrainArenaPosition(end);return end}const dir=aimDirection(caster),b=origin.x*dir.x+origin.z*dir.z,c=origin.x*origin.x+origin.z*origin.z-moveLimit*moveLimit,dist=Math.max(.1,-b+Math.sqrt(Math.max(0,b*b-c))),end={x:origin.x+dir.x*dist,y:floorY,z:origin.z+dir.z*dist};constrainArenaPosition(end);end.distance=Math.hypot(end.x-origin.x,end.z-origin.z);return end}
function aimRangeToHit(origin,hit,card){return Math.min(cardRange(card),Math.max(.1,Math.hypot(hit.x-origin.x,hit.z-origin.z)))}
...
function skillHitCaster(h){return{pos:{x:h.origin.x,y:h.origin.y,z:h.origin.z},rot:{y:Math.atan2(-h.dir.x,-h.dir.z)},element:h.element,fxOrigin:true}}
function markHitOnce(h,id){h.hitIds=h.hitIds||{};if(h.hitIds[id])return false;h.hitIds[id]=true;return true}
function travelPoint(h,range){return{x:h.origin.x+h.dir.x*range,y:h.hit.y,z:h.origin.z+h.dir.z*range}}
function travelCollisionTargets(h,t){if(!h?.card||h.card.stream)return[];const area=typeof areaTelegraphCard==="function"&&areaTelegraphCard(h.card,h.element);if(area&&!h.card.wave)return[];const total=Math.max(.1,h.range||0),created=h.createdAt??t,impact=h.impactAt??t,span=Math.max(1,impact-created),current=clamp(total*((t-created)/span),0,total),prev=h.travelRangeSeen||0;if(current<=prev+.05)return[];h.travelRangeSeen=current;const hit=travelPoint(h,current),targets=pathTargetCandidates({card:h.card,element:h.element,origin:h.origin,dir:h.dir,range:current,hit,targetId:h.targetId||""});return targets.filter(x=>x.lineHit&&!h.hitIds?.[x.id]&&x.along>=prev-targetHitboxRadius(x.p))}
function resolveTravelHits(h,targets,e){if(!targets.length)return false;const caster=skillHitCaster(h),impactCard={...h.card,resolvedImpact:true};if(h.card.wave){targets.forEach(t=>{if(markHitOnce(h,t.id)){hitTarget(t,h.card,e);skillEffect(impactCard,t,e,caster,t.hit)}});return false}const t=targets[0];if(markHitOnce(h,t.id)){hitTarget(t,h.card,e);skillEffect(impactCard,t,e,caster,t.hit)}return true}
function resolveSkillHit(h){const e=elements[h.element]||elements.fire,targets=targetsOnStoredPath(h).filter(t=>!h.hitIds?.[t.id]),area=typeof areaTelegraphCard==="function"&&areaTelegraphCard(h.card,h.element),wave=!!h.card?.wave,hitList=wave?targets.filter(t=>t.lineHit):area?targets.filter(t=>t.impactHit):targets.slice(0,1),caster=skillHitCaster(h),impactCard={...h.card,resolvedImpact:true};if(hitList.length){hitList.forEach(t=>{if(markHitOnce(h,t.id))hitTarget(t,h.card,e)});skillEffect(impactCard,hitList[0],e,caster,wave?h.hit:area?h.hit:hitList[0].hit)}else skillEffect(impactCard,null,e,caster,h.hit)}
function updatePendingHits(t){if(!pendingHits.length)return;const keep=[];pendingHits.forEach(h=>{const e=elements[h.element]||elements.fire;if(resolveTravelHits(h,travelCollisionTargets(h,t),e))return;if(t<h.impactAt)keep.push(h);else resolveSkillHit(h)});pendingHits=keep}
...
function startPsychicLiftControl(target,card,e){
  ...
  const timer=setInterval(()=>{
    ...
    p.stunUntil=Math.max(p.stunUntil||0,t+135);
    p.hitId=(p.hitId||0)+1;
...
function hitTarget(target,card,e){
  ...
  if(landed){
    ...
    if((card.stun||parryBonus)&&!(p.stunUntil>t)){
      const baseStun=card.stun||0,resisted=(p.ccResistUntil||0)>t&&baseStun>0,stunDur=((resisted?baseStun*.5:baseStun)+parryBonus)*1.5;
  ...
  if(me){me.kills=local.kills||0;me.damageDone=local.damageDone||0;me.parryBonusUntil=local.parryBonusUntil||0;me.parryBonusStun=local.parryBonusStun||0;me.parryTarget=local.parryTarget||""}
  p.hitId=(p.hitId||0)+1;
