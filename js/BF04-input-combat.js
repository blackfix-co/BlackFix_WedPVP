...
function chargePercent(){if(!local.charge)return 0;const max=local.chargeMode==="sun"?3:local.chargeMode==="psychicLift"?1.1:local.chargeMode==="stream"?1.6:local.chargeMode==="parry"?1.2:2;return clamp(Math.round(((local.chargeTime||0)/max)*100),0,100)}
...
const parrySlow=local.chargeMode==="parry"?.55:1,base=(t<local.speedUntil?9:6.5)*(local.moveBase||1)*local.move*(t<local.slowUntil?.55:1)*parrySlow;let forward=0,strafe=0;
...
function clearChargeSkill(){local.charge=false;local.chargeMode="";local.chargeTime=0;local.chargeTick=0;local.chargeFxTick=0;local.parryUntil=0;if($("cards")&&!$("hud").classList.contains("hidden"))renderCards();else renderChargeMeter()}
function startChargeSkill(){if(!local.alive||now()<local.stunUntil)return;const e=elements[local.element]||elements.fire,cards=getCards(e),card=cards[selectedCard];if(!card)return;if(card.type==="방어"){log("방어 카드는 우클릭 기능이 없습니다.");return}if(cardLocked(selectedCard)){log("해당 카드가 봉인되어 있습니다.");return}if(local.element==="psychic"&&selectedCard===3)local.chargeMode="psychicLift";else if(local.element==="dragon")local.chargeMode="dragonBeam";else if(card.type==="공격")local.chargeMode="stream";else if(card.type==="CC기")local.chargeMode="parry";else if(local.element==="fire"&&selectedCard===3)local.chargeMode="sun";else local.chargeMode="charge";if((local.chargeMode==="sun"||local.chargeMode==="psychicLift")&&(local.cooldowns[3]||0)>0){log("궁극기 쿨타임입니다.");local.chargeMode="";return}local.charge=true;local.chargeCard=selectedCard;local.chargeTime=0;local.chargeTick=0;local.chargeFxTick=0;if(local.chargeMode==="parry")local.parryUntil=now()+220;renderCards();chargePreviewEffect(true);log(local.chargeMode==="stream"?"지속 공격 시작":local.chargeMode==="dragonBeam"?"화염 빔 시작":local.chargeMode==="parry"?"패링 태세":local.chargeMode==="sun"?"태양 차징 시작":local.chargeMode==="psychicLift"?"염동 포획 차징":"마나 차징 시작")}
function releaseChargeSkill(){if(!local.charge)return;const held=local.chargeTime||0,cardIndex=local.chargeCard,mode=local.chargeMode;clearChargeSkill();if(mode==="sun"&&cardIndex===3&&held>.22)throwFireSun(held);else if(mode==="psychicLift"&&cardIndex===3&&held>.16)armPsychicLift(held);else if(mode==="charge"&&held>.28)throwChargedCard(cardIndex,held)}
function updateChargeSkill(dt,t){if(!local.charge||!scene){renderChargeMeter();return}renderChargeMeter();if(local.chargeMode==="stream"||local.chargeMode==="dragonBeam"){if(local.chargeMode==="dragonBeam"&&local.chargeTime>3){clearChargeSkill();log("화염 빔 과열");return}local.chargeFxTick-=dt;local.chargeTick-=dt;if(local.chargeFxTick<=0){chargePreviewEffect(false);local.chargeFxTick=.065}if(local.chargeTick<=0){continuousAttackTick();local.chargeTick=.16}}else if(local.chargeMode==="parry"){local.parryUntil=t+220;local.chargeFxTick-=dt;if(local.chargeFxTick<=0){chargePreviewEffect(false);local.chargeFxTick=.12}}else if(local.chargeMode==="psychicLift"){local.chargeFxTick-=dt;if(local.chargeFxTick<=0){chargePreviewEffect(false);local.chargeFxTick=.075}}else{local.chargeFxTick-=dt;if(local.chargeFxTick<=0){chargePreviewEffect(false);local.chargeFxTick=.11}}}
function chargePreviewEffect(burst=false){if(!scene)return;const e=elements[local.element]||elements.fire,color=e.color,sy=Math.sin(local.rot.y),cy=Math.cos(local.rot.y),front={x:local.pos.x-sy*1.1,y:local.pos.y+.25,z:local.pos.z-cy*1.1};if(local.chargeMode==="sun"){const s=clamp(.55+(local.chargeTime||0)*.45,.55,1.45),sun=new THREE.Mesh(new THREE.SphereGeometry(s,24,16),effectMaterial(0xffb23a,.42));addEffect(sun,{x:front.x,y:front.y+1.1,z:front.z},burst?.55:.18,1.08);ringAt({x:front.x,y:front.y+1.1,z:front.z},0xfff08a,s*.9,burst?.55:.22);burstShape({x:front.x,y:front.y+1.1,z:front.z},0xff6a2a,burst?16:4,"flame",s*1.5)}else if(local.chargeMode==="psychicLift"){const pct=clamp((local.chargeTime||0)/1.1,.08,1),core=new THREE.Mesh(new THREE.IcosahedronGeometry(.28+pct*.34,1),effectMaterial(0xd6a6ff,.48));addEffect(core,{x:front.x,y:front.y+.72,z:front.z},burst?.42:.16,1.12);for(let i=0;i<3;i++){const ring=new THREE.Mesh(new THREE.TorusGeometry((.48+pct*.38)+i*.18,.024,8,72),effectMaterial(i?color:0xffffff,.52));ring.rotation.x=Math.PI/2+i*.38;ring.rotation.z=performance.now()*.002+i;addEffect(ring,{x:front.x,y:front.y+.72+i*.04,z:front.z},burst?.45:.18,1.22)}burstShape({x:front.x,y:front.y+.72,z:front.z},color,burst?18:5,"spark",.9+pct*.75)}else if(local.chargeMode==="stream"){const tip={x:front.x,y:front.y-.25,z:front.z};burstShape(tip,color,burst?12:4,local.element==="fire"?"flame":local.element==="earth"?"rock":local.element==="lightning"?"bolt":"bubble",burst?1.2:.65);if(burst)ringAt(tip,color,.55,.35)}else if(local.chargeMode==="parry"){const guard=new THREE.Mesh(new THREE.TorusGeometry(1.05,.055,8,64),effectMaterial(0xffffff,.62));guard.rotation.x=Math.PI/2;addEffect(guard,{x:local.pos.x,y:local.pos.y-.55,z:local.pos.z},burst?.45:.18,1.45);burstShape(front,0xffffff,burst?10:3,"spark",.85)}else{const aura=new THREE.Mesh(new THREE.TorusGeometry(.8,.045,8,56),effectMaterial(color,.6));aura.rotation.x=Math.PI/2;addEffect(aura,{x:local.pos.x,y:local.pos.y-.95,z:local.pos.z},burst?.55:.25,1.7)}}
function continuousAttackTick(){const e=elements[local.element]||elements.fire,base=getCards(e)[local.chargeCard];let cost=Math.max(3,Math.round((base.mana||10)*.42)),card={...base,name:"연속 "+base.name,mana:0,cd:.1,damage:Math.max(3,Math.round((base.damage||10)*.42)),stream:true};if(local.chargeMode==="dragonBeam"){cost=3;card={...base,name:"화염 빔",mana:0,cd:.1,damage:5,burn:2,stream:true,beam:true}}if(local.mana<cost){clearChargeSkill();log("마나 부족으로 지속 공격이 멈췄습니다.");return}local.mana=clamp(local.mana-cost,0,local.maxMana);if(card.burn)card.burn=Math.max(2,Math.round(card.burn*.45));if(card.poison)card.poison=Math.max(2,Math.round(card.poison*.45));if(card.drain)card.drain=Math.min(.16,card.drain);if(card.slow)card.slow=Math.min(.65,card.slow*.45);const target=findTarget(card),queued=queueSkillHit(card,e,target);recordSkillFx(card,target,queued);useCastMotion(card);patch({mana:Math.round(local.mana),parryUntil:0,cooldowns:[...(local.cooldowns||[0,0,0,0])]})}
...
function castDragonQ(){if(local.element!=="dragon"||!local.alive||now()<local.stunUntil)return;const t=now();if(t<(local.dragonQCdUntil||0)){log(`용의 불 쿨타임 ${((local.dragonQCdUntil-t)/1000).toFixed(1)}초`);return}const cost=14;if(local.mana<cost){log(`마나 부족: ${Math.round(local.mana)} / ${cost}`);return}const e=elements.dragon,card={name:"용의 불 발사",type:"공격",mana:cost,cd:2.5,damage:18,burn:7,dragonQ:true};local.mana-=cost;local.dragonQCdUntil=t+2500;const target=findTarget(card),queued=queueSkillHit(card,e,target);recordSkillFx(card,target,queued);useCastMotion(card);patch({mana:Math.round(local.mana),dragonQCdUntil:local.dragonQCdUntil});log("용의 불 발사")}
function throwChargedCard(cardIndex,held){const e=elements[local.element]||elements.fire,base=getCards(e)[cardIndex];if(!base)return;if((local.cooldowns[cardIndex]||0)>0){log("차징 카드 쿨타임입니다.");return}const charge=clamp(held/2,.18,1),cost=Math.min(local.mana,Math.round((base.mana||20)*(1+charge*.25)));if(local.mana<cost){log(`마나 부족: ${Math.round(local.mana)} / ${cost}`);return}const card={...base,name:"차징 "+base.name,mana:cost,damage:Math.round((base.damage||0)*(1+charge*.45)),charge};if(card.burn)card.burn=Math.round(card.burn*(1+charge*.35));if(card.poison)card.poison=Math.round(card.poison*(1+charge*.35));if(card.slow)card.slow=Math.min(4,card.slow*(1+charge*.12));local.mana=clamp(local.mana-cost,0,local.maxMana);local.cooldowns[cardIndex]=Math.max(.8,(base.cd||1)*local.cdMul);const target=findTarget(card),queued=card.damage>0?queueSkillHit(card,e,target):null;if(!queued)skillEffect(card,target,e);recordSkillFx(card,target,queued);useCastMotion(card);patch({mana:Math.round(local.mana),cooldowns:[...(local.cooldowns||[0,0,0,0])],coins:local.coins,totalCoins:local.totalCoins||0,kills:local.kills||0});renderCards();log(card.name+" 사용")}
...
function cardHitRadius(card,along=0,id=local.element||"fire"){let base=4.8;if(id==="lightning"||id==="light"||id==="hacker")base=3.6;if(id==="wind")base=4.2;if(id==="earth")base=5.7;if(id==="darkness")base=5.4;if(id==="poison")base=4.9;if(id==="dragon")base=5.6;if(id==="psychic")base=4.5;if(id==="summoner")base=4.2;if(card?.psychicLift)base=5.4;if(card?.stream)base=Math.max(3.7,base-.4);if(card?.beam)base=5.4;if(crowdControlCard(card))base+=id==="light"||id==="wind"||id==="hacker"?1.3:2;if(isUltimateCard(card))base+=id==="water"?5:id==="dragon"?7:id==="psychic"?5:id==="wind"?4:id==="darkness"?4:id==="poison"?3.6:3;if(card?.sun)base=12.5;if(card?.charge)base+=card.charge*1.2;return base+Math.min(5.2,Math.max(0,along)*.04)}
function impactHitRadius(card,id=local.element||"fire"){if(card?.shield||card?.heal)return 0;if(card?.psychicLift)return 8.5;if(card?.sun)return 15+(card.charge||0)*2.5;if(card?.dragonSlam)return 16;if(card?.gravity)return 13;if(card?.roundSeal)return 12;if(isUltimateCard(card))return id==="dragon"?14:id==="psychic"?13:id==="water"?13:id==="wind"?12:id==="darkness"?13:id==="poison"?11:id==="earth"?10:id==="fire"?11:8.5;if(crowdControlCard(card))return id==="dragon"?8:id==="psychic"?7:id==="hacker"?6.2:id==="fire"?8.5:id==="earth"?7.2:id==="poison"?7:id==="lightning"?5.8:id==="light"?6.4:6.8;return id==="dragon"?6:id==="fire"||id==="darkness"?5.5:4.6}
function areaTelegraphCard(card,id=local.element||"fire"){if(!card||card.stream||card.type==="방어"||card.heal||card.shield)return false;if(card.areaTelegraph||card.sun||card.gravity||card.roundSeal||card.dragonSlam||card.spiritGrand||card.psychicLift||card.devour)return true;if(isUltimateCard(card))return true;if(card.type==="CC기")return true;return id==="lightning"&&card.type==="공격"}
function aimEndpoint(card=null,caster=local){const dir=aimDirection(caster),range=cardRange(card),pos=caster.pos||local.pos,origin=typeof castOrigin==="function"?castOrigin(caster):{x:pos.x,y:(pos.y||2)+.55,z:pos.z};return{x:origin.x+dir.x*range,y:origin.y,z:origin.z+dir.z*range}}
...
function queueSkillHit(card,e,target=null){const id=local.element||"fire",dir=aimDirection(local),range=cardRange(card),origin=typeof castOrigin==="function"?castOrigin(local):{x:local.pos.x,y:local.pos.y+.55,z:local.pos.z},hit=skillEndpoint(target,local,card),dist=target?.along||Math.hypot(hit.x-origin.x,hit.z-origin.z),delay=skillImpactDelay(id,card,dist),h={id:selfId+"-"+now()+"-"+Math.random().toString(36).slice(2,7),card:{...card},element:id,origin,hit,dir,range,targetId:target?.id||"",createdAt:now(),impactAt:now()+delay*1000,delay};pendingHits.push(h);if(typeof travelTelegraphEffect==="function")travelTelegraphEffect(h,e);return h}
function resolveSkillHit(h){const e=elements[h.element]||elements.fire,targets=targetsOnStoredPath(h),area=typeof areaTelegraphCard==="function"&&areaTelegraphCard(h.card,h.element),hitList=area?targets:targets.slice(0,1),caster={pos:{x:h.origin.x,y:h.origin.y,z:h.origin.z},rot:{y:Math.atan2(-h.dir.x,-h.dir.z)},element:h.element,fxOrigin:true},impactCard={...h.card,resolvedImpact:true};if(hitList.length){hitList.forEach(t=>hitTarget(t,h.card,e));skillEffect(impactCard,hitList[0],e,caster,area?h.hit:hitList[0].hit)}else skillEffect(impactCard,null,e,caster,h.hit)}
